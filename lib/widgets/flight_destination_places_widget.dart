...
/// Flight destination place picker card (same layout as flight origin places).
class FlightDestinationPlacesWidget extends StatelessWidget {
  final List<CarPickupPlace> places;
  final void Function(CarPickupPlace place)? onPlaceSelected;
  final bool isFromChatHistory;

  const FlightDestinationPlacesWidget({
    super.key,
...
class _FlightDestinationPlaceRow extends StatelessWidget {
  final String label;
  final VoidCallback? onTap;

  const _FlightDestinationPlaceRow({
    required this.label,
    this.onTap,
  });
  ...
  Widget build(BuildContext context) {
    final row = Container(
      ...
      decoration: BoxDecoration(
        color: FlightDestinationPlacesWidget._rowBackground,
      ...
      child: Text(
        ...
        style: AppTextStyles.bodyText.copyWith(
          ...
          height: 1.4,
          color: FlightDestinationPlacesWidget._labelColor,
        ),
      ),
    );
